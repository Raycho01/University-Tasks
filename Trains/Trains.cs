...
namespace ConsoleApplication1
{
    ...
    {
        private const Train[] trains = [Train("New York", new TimeSpan(8, 30, 0), 50),
                                        Train("Boston", new TimeSpan(10, 15, 0), 35),
                                        Train("Washington D.C.", new TimeSpan(13, 45, 0), 70)];
        ...
        private Double calculateTicketPriceBasedOnTrip(bool isRoundTrip)
        {
            ...
            {
                priceOfTicket *= 2;
                priceOfTicket *= 0.9m; // 10% discount for round-trip
            }
            return priceOfTicket;
        }
            ...
            foreach (Train train in trains)
            {
                ...
                if (train.departureTime == desiredTimeOfLeaving)
                {
                    ...
                    if (train.seatsAvailable >= ticketsCount)
                    {
                        ...
                        message = $"Reservation successful for {ticketsCount} seat(s) on train to {train.destination} leaving at {train.departureTime}";
        ...
        public TimeSpan getTrainsHourOfLeaving(string destination)
        {
            foreach (Train train in trains)
            {
                ...
                {
                    Console.WriteLine($"Train to {train.destination} is leaving at {train.departureTime}");
                    return train.departureTime;
                }
            }
            Console.WriteLine($"There are no trains with destination - {train.destination}");
            return TimeSpan();
        }
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            double fullPrice = manager.getPriceFor("19:30", CardType.Elder, 1, "Tokyo", false);
